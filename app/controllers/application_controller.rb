class ApplicationController < ActionController::Base
  ...
  private
   ...
   def current_manager
     @current_manager ||= current_person if person_signed_in? and current_person.class.name == "Manager"
   end
    ...
    def current_receptionist
      @current_receptionist ||= current_person if person_signed_in? and current_person.class.name == "Receptionist"
    end

    def current_lifeguard
      @current_lifeguard ||= current_person if person_signed_in? and current_person.class.name == "Lifeguard"
    end

    def current_trainer
      @current_trainer ||= current_person if person_signed_in? and current_person.class.name == "Trainer"
    end

    def manager_logged_in?
      @manager_logged_in ||= person_signed_in? and current_manager
    end
    ...
    def receptionist_logged_in?
      @receptionist_logged_in ||= person_signed_in? and current_receptionist
    end

    def lifeguard_logged_in?
      @lifeguard_logged_in ||= person_signed_in? and current_lifeguard
    end

    def trainer_logged_in?
      @trainer_logged_in ||= person_signed_in? and current_trainer
    end
    ...
    def require_person_type(person_type)
      if(person_type == :manager and !manager_logged_in?) or
        (person_type == :client and !client_logged_in?) or
        (person_type == :receptionist and !receptionist_logged_in?) or
        (person_type == :lifeguard and !lifeguard_logged_in?) or
        (person_type == :trainer and !trainer_logged_in?)
